...
_G.Settings= {
    Name 		        = "*emaRenkoATR",
    ema_period          = 14,                   -- ѕериод расчета ema
    smoothLines         = 0,
    period              = 28,                   -- ѕериод расчета ATR
    k                   = 1.0,                  -- размер скольз€щего фильтра, используемый при вычислении размера блока от величины ATR как k*ATR
    data_type           = 1,                    -- 0 - Close; 1 - High|Low
    br_size             = 0,                    -- ‘иксированный размер шага. ≈сли задан, то строитс€ по указанному размеру (в пунктах)
    recalc_brick        = 0,                    -- ѕересчитывать размер блока каждый период-бар
    shift_limit         = 0,                    -- —двигать границу по пересчитанному размеру блока
    min_recalc_brick    = 0,                    -- ћинимизировать размер блока при пересчете
    --ƒл€ установки значени€, необходимо поставить * перед выбранным вариантом.
    brickType           = '*ATR; Std; Fix',     -- “ип расчета Renko; ATR; Std - стандартное отклонение; Fix - фиксированный размер, заданный в br_size
    std_ma_method       = 'SMA',
    line = {
        {
            Name  = 'emaRenko',
