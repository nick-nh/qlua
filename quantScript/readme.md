...
upd. Добавил переменную dirTradeType = 2 -- 1 - направление из ТОС, 2 - направление считается как дельта от прошлой цены. Если цена снизилась, то продажа, если повысилась, то покупка. Т.о. покупка/продажа не берется из ТОС, а определяется от движения цены. Это дате более точное определение дельты.
