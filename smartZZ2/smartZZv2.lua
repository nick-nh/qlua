...
-- version 2, classic Zig-Zag algorithm
...
function GetPeak(index, offset, Peak, ZZLevels)
    ...
    --local sizeOfZZLevels = #ZZLevels
    --peakIndex = ZZLevels[2][sizeOfZZLevels[2]-offset];
    --if peakIndex == 0 or peakIndex == nil then
    --    return -1;
    --end
    --return peakIndex;
end
...
function cached_ZZ()
	...
	local CH={} -- значения максимумов
	local CL={} -- значения минимумов
	
	local HighMapBuffer={} -- знечения максимов предшествующего движения
	local LowMapBuffer={} -- знечения минимумов предшествующего движения
	...
	return function(ind, Fsettings, numberOfLiines)
		...
		local Deviation = Fsettings.Deviation or 12
		local Backstep = Fsettings.Backstep or 9
		...
		--WriteLog ("HighMapBuffer[index-1] "..tostring(HighMapBuffer[index-1]));
		--WriteLog ("LowMapBuffer[index-1] "..tostring(LowMapBuffer[index-1]));
		...
		if showZZLevels == 1 then
			for nn = 1, maxNumberOfZZLevelsLines*2*2 do
				SetValue(lineIndex[numberOfLiines-nn+1]["index"], numberOfLiines-nn+1, nil)
        ...
        if index == lastindex then
            ...
            if ZZLevels[sizeOfZZLevels] ~= nil then

                local lastZZ = ZZLevels[sizeOfZZLevels]["val"]
                local lastZZ_i = ZZLevels[sizeOfZZLevels]["index"]
                ...
                if searchMode == searchPeak and L(index) < lastZZ then
					...
					--WriteLog ("--------------------------");
					--WriteLog ("index update low");
					--WriteLog ("HighMapBuffer[lastZZ_i] "..tostring(HighMapBuffer[lastZZ_i]));
					--WriteLog ("LowMapBuffer[lastZZ_i] "..tostring(LowMapBuffer[lastZZ_i]));
					--WriteLog ("old T("..lastZZ_i.."); "..isnil(toYYYYMMDDHHMMSS(T(lastZZ_i))).." "..tostring(lastZZ))
					SetValue(lastZZ_i, 1, nil)
                    ReplaceLastPeak(index, L(index), Peak, 0, ZZLevels);
				end
                if searchMode == searchLawn and H(index) > lastZZ then
                    Peak[lastZZ_i] = nil
                    HighMapBuffer[lastZZ_i] = 0
                    HighMapBuffer[index] = H(index)
					--WriteLog ("--------------------------");
					--WriteLog ("index update high");
					--WriteLog ("HighMapBuffer[lastZZ_i] "..tostring(HighMapBuffer[lastZZ_i]));
					--WriteLog ("LowMapBuffer[lastZZ_i] "..tostring(LowMapBuffer[lastZZ_i]));
					--WriteLog ("old T("..lastZZ_i.."); "..isnil(toYYYYMMDDHHMMSS(T(lastZZ_i))).." "..tostring(lastZZ))
					SetValue(lastZZ_i, 1, nil)
                    ReplaceLastPeak(index, H(index), Peak, 0, ZZLevels);
                end
            end  
            ...
            if ZZLevels[sizeOfZZLevels] ~= nil then
                SetValue(ZZLevels[sizeOfZZLevels]["index"], 1, nil)
            end
            ...
            --WriteLog ("---------------------------------");
            --WriteLog ("peak -3 "..tostring(last_peak_i));
            --WriteLog ("T("..last_peak_i.."); "..isnil(toYYYYMMDDHHMMSS(T(last_peak_i))));
            if ZZLevels[sizeOfZZLevels-2] ~= nil then
                --WriteLog ("ZZ peak -3 "..tostring(ZZLevels[sizeOfZZLevels-2]["val"]));
                --WriteLog ("iZZ peak -3 "..tostring(ZZLevels[sizeOfZZLevels-2]["index"]));
            end
            ...
            --WriteLog ("searchMode -3 "..tostring(searchMode));
            ...
            for i = start, index-1, 1 do
				...
				--WriteLog ("");				
                --WriteLog ("Set MAP----------------");
                --WriteLog ("T("..i.."); "..isnil(toYYYYMMDDHHMMSS(T(i))));
                --WriteLog ("L "..tostring(L(i)));
                --WriteLog ("H "..tostring(H(i)));
                --WriteLog ("lastlow "..tostring(lastlow));
                --WriteLog ("lasthigh "..tostring(lasthigh));
                
                -- get the lowest low for the last depth is
                val = math.min(unpack(CL, range, i));
                --WriteLog ("val L "..tostring(val));
                if val == lastlow then
					...
					--WriteLog ("cond L "..tostring((CL[i] - val) > (min_price_step * Deviation)));
                    if (CL[i] - val) > (min_price_step * Deviation) then
                        val = nil;
                    else
                        -- check for the previous backstep lows
                        for k = i - 1, i - Backstep + 1, -1 do
							--WriteLog ("Set Back----------------");
							--WriteLog ("T("..k.."); "..isnil(toYYYYMMDDHHMMSS(T(k))));
							--WriteLog ("LowMapBuffer[k] "..tostring(LowMapBuffer[k]));
				...
				--WriteLog ("new LowMapBuffer[i] "..tostring(LowMapBuffer[i]));
                
                -- get the highest high for the last depth is
                val = math.max(unpack(CH, range, i));
                --WriteLog ("----------------");
                --WriteLog ("val H "..tostring(val));
                if val == lasthigh then
                    ...
                    lasthigh = val;
                    -- if current low is higher for more than Deviation pips, ignore
					--WriteLog ("cond H "..tostring((val - CH[i]) > (min_price_step * Deviation)));
                    if (val - CH[i]) > (min_price_step * Deviation) then
                        val = nil;
                    else
                        -- check for the previous backstep lows
                        for k = i - 1, i - Backstep + 1, -1 do
							--WriteLog ("Set Back----------------");
							--WriteLog ("T("..k.."); "..isnil(toYYYYMMDDHHMMSS(T(k))));
							--WriteLog ("HighMapBuffer[k] "..tostring(HighMapBuffer[k]));
				...
				--WriteLog ("new HighMapBuffer[i] "..tostring(HighMapBuffer[i]));
                
            end
			
			--WriteLog ("  ");
			--WriteLog ("Set extr --------------------------");
			--WriteLog ("HighMapBuffer[last_peak_i] "..tostring(HighMapBuffer[last_peak_i]));
            --WriteLog ("LowMapBuffer[last_peak_i] "..tostring(LowMapBuffer[last_peak_i]));
			--WriteLog ("HighMapBuffer[index] "..tostring(HighMapBuffer[index]));
            --WriteLog ("LowMapBuffer[index] "..tostring(LowMapBuffer[index]));
			...
			--WriteLog ("peak_count "..tostring(peak_count));
			--WriteLog ("last_peak_i "..tostring(last_peak_i));
			--WriteLog ("start "..tostring(start));
			--WriteLog ("searchMode "..tostring(searchMode));
			--WriteLog ("--------------------------");
            ...
            for i = start, index-1, 1 do
                ...
                elseif searchMode == searchPeak then
                    if (LowMapBuffer[i] ~= 0 and LowMapBuffer[i] < last_peak) then --and HighMapBuffer[i] == 0
                        SetValue(last_peak_i, 1, nil)
						--WriteLog ("--------------------------");
						--WriteLog ("update low");
						--WriteLog ("HighMapBuffer[i] "..tostring(HighMapBuffer[i]));
						--WriteLog ("LowMapBuffer[i] "..tostring(LowMapBuffer[i]));
						--WriteLog ("old T("..ZZLevels[sizeOfZZLevels]["index"].."); "..isnil(toYYYYMMDDHHMMSS(T(ZZLevels[sizeOfZZLevels]["index"]))).." "..tostring(ZZLevels[sizeOfZZLevels]["val"]))
		...
		if  index == Size() and sizeOfZZLevels > 4 then
			...
			--WriteLog ("X "..tostring(ZZLevels[sizeOfZZLevels-3]["val"]))
			--WriteLog ("A "..tostring(ZZLevels[sizeOfZZLevels-2]["val"]))
			--WriteLog ("B "..tostring(ZZLevels[sizeOfZZLevels-1]["val"]))
			--WriteLog ("C "..tostring(ZZLevels[sizeOfZZLevels]["val"]))
			...
			if showZZLevels == 1 then
                ...
                for j = 1, numberOfZZLevels do
                    if ZZLevels[sizeOfZZLevels-j+1] ~= nil then
                        ...
                        SetValue(ZZLevels[sizeOfZZLevels-j+1]["index"], numberOfLiines-j+1+add, ZZLevels[sizeOfZZLevels-j+1]["val"])
                        lineIndex[numberOfLiines-j+1+add]["val"] = ZZLevels[sizeOfZZLevels-j+1]["val"]
                        lineIndex[numberOfLiines-j+1+add]["index"] = ZZLevels[sizeOfZZLevels-j+1]["index"]
