...
Для некоторых модулей есть свои файлы параметров. Зачем они нужны. В таблице settings модуля алгоритма указаны значения по-умолчанию для всех бумаг и интервалов. Но для каждой бумаги и интервала лучше использовать сови параметры. Скажем на 15 минутах свои параметры, а н дневках свои. Также и для разных бумаг.
Поэтому, для примера, в алгоритме rangeMonitor есть файл csv с настройками, для некоторых бумаг и интервалов. Можно добаялять, удалять, менять своими значениями.
...
Чтобы открыть сделки по инструменту, надо дважды щелкнуть по имени в таблице монитора. При открытии таблицы, сделки будут распределены по ФИФО. Незакрытие будут выделены зеленым цветом.

Вот и все.

Теперь как все это заставить лететь:
Обновлено: теперь все можно положить в одну папку, скажем ScriptMonitor, расположенныю где угодно (ранее надо было создавать дополнительные папки).

Скрипт и модули кладем собираем в онду папку, кладем куда угодно. LuaIndicators там где установлен Квик.
